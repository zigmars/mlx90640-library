...
void ExtractGainParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
    ...
    gainEE = eeData[48];
    if (gainEE > 32767) { gainEE = gainEE - 65536; }
...
void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
    ...
    for (int i = 0; i < 24; i++) {
        for (int j = 0; j < 32; j++) {
            ...
            if (alphaTemp[p] > 31) { alphaTemp[p] = alphaTemp[p] - 64; }
...
void ExtractKtaPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
    ...
    if (KtaRoCo > 127) { KtaRoCo = KtaRoCo - 256; }
    KtaRC[0] = KtaRoCo;

    KtaReCo = (eeData[54] & 0x00FF);
    if (KtaReCo > 127) { KtaReCo = KtaReCo - 256; }
    KtaRC[2] = KtaReCo;

    KtaRoCe = (eeData[55] & 0xFF00) >> 8;
    if (KtaRoCe > 127) { KtaRoCe = KtaRoCe - 256; }
    KtaRC[1] = KtaRoCe;

    KtaReCe = (eeData[55] & 0x00FF);
    if (KtaReCe > 127) { KtaReCe = KtaReCe - 256; }
    KtaRC[3] = KtaReCe;
    ...
    for (int i = 0; i < 24; i++) {
        for (int j = 0; j < 32; j++) {
            ...
            if (ktaTemp[p] > 3) { ktaTemp[p] = ktaTemp[p] - 8; }
